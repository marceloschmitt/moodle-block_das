<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
	<title>DAS - Relatório Geral de Notas</title>
	<link rel="stylesheet" type="text/css" href="assets/css/normalize.css">
    <link rel="stylesheet" type="text/css" href="assets/css/das-general-report.css">
    <link rel="stylesheet" type="text/css" href="assets/css/bootstrap.css">
    <link rel="stylesheet" type="text/css" href="assets/css/animate.css">
    <link rel="stylesheet" type="text/css" href="assets/css/style.css">

</head>

<body>
<div class="das-general-report">
<p class="das-subtitle">Relatório Geral de notas do DAS</p>

<form method="GET" action="">
	<div class="das-filter-users">
	<p>Selecionar filtro por:</p>
		
		<div class="das-select-topic">
			<p>Tópico</p> 
			<select name="das-select-topic-user" onchange='this.form.submit()'>
				<option>Selecionar...</option>
				<option value="Tópico1">Tópico 1</option>
				<option>Tópico 2</option>
				<option>Tópico 3</option>
				<option>Tópico 4</option>
			</select>
			<noscript><input type="submit" value="Submit"></noscript>
		</div>

		<div class="das-select-concept">
			<p>Conceito</p>
			<select name="das-select-notes" onchange='this.form.submit()'>
				<option>Selecionar...</option>
				<option value="A">A</option>
				<option value="B">B</option>
				<option value="C">C</option>
				<option value="D">D</option>
				<option value="FF">FF</option>
			</select>
		
		</div>

		<div class="das-select-note">
		<p>Nota</p> 
			<select name="das-select-note" onchange='this.form.submit()'>
				<option>Selecionar...</option>
				<option>1</option>
				<option>2</option>
				<option>3</option>
				<option>4</option>
				<option>5</option>
				<option>6</option>
				<option>7</option>
				<option>8</option>
				<option>9</option>
				<option>10</option>
			</select>

		</div>

		<div class="das-select-period">
		<p>Período</p>
			<select name="das-select-period" onchange='this.form.submit()'>
				<option>Selecionar...</option>
				<option>1 dia</option>
				<option>2 dias</option>
				<option>3 dias</option>
				<option>4 dias</option>
				<option>5 dias</option>
				<option>6 dias</option>
				<option>1 semana</option>
				<option>2 semanas</option>
			</select>
		</div>
	

	</div>
	</form>

	<div class="das-table-select">
		<table border="1" id="das-table-students-col">
   			<tr class="das-color-ccc"> 
   				<td><p class="das-title">Aluno</td>
				<th class="das-color-9" colspan="4"><p class="das-title">Integração de Mídias na Educação</p> </th>
				<td><p class="das-title">Conceito</p></td>
			</tr>

   			<tr class="das-color-ccc" >
   				<td class="das-color-9">&nbsp;</td>
				<td>A1</td>
				<td>A2</td>
				<td>A3</td>
				<td>A4</td>
				<td class="das-color-9">&nbsp;</td>
			</tr>

			<tr class="das-color-ccc" id="A"> <!-- Alunos -->
   				<td class="das-color-orange"><p>Anita Raquel da Silva</p></td>
				<td><p>6</p></td>
				<td ><p>9</p></td>
				<td><p>4</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc" id="B">
				<td class="das-color-orange"><p>Antônio Marcondes Lima</p></td>
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>6</p></td>
				<td><p>4</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc" id="B">
				<td class="das-color-orange"><p>Bruna Gomes</p></td>
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>5</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc" id="B">
				<td class="das-color-orange"><p>Cristiane de Souza Abreu</p></td>
				<td><p>4</p></td>
				<td><p>9</p></td>
				<td><p>4</p></td>
				<td><p>3</p></td>
				<td class="das-color-orange"><p><b>D</b></p></td>
			</tr>

			<tr class="das-color-ccc" id="C">
				<td class="das-color-orange"><p>Daniela Guimarães Leal</p></td>
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>7</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>B</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td class="das-color-orange"><p>Givaldo Batista Medeiros</p></td>
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>7</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc"> 
				<td class="das-color-orange"><p>Kelly Galarza</p></td>
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>5</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td class="das-color-orange"><p>Liêge Barbosa</p></td>
				<td><p>7</p></td>
				<td><p>9</p></td>
				<td><p>7</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td class="das-color-orange"><p>Magali Inês Passini</p></td>
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>8</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>B</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td class="das-color-orange"><p>Raul Araújo Neto</p></td>
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>6</p></td>
				<td><p>3</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>
			
			<tr class="das-color-ccc">
				<td class="das-color-orange"><p>Tássia Priscila Fagundes</p></td>
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>6</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			
		<table border="1" id="das-students-table-col-notes">
			<tr>
				<th colspan="4" class="das-color-9"><p class="das-title">Projeto e Produção de Material</p> </th>
				<td class="das-color-ccc"><p class="das-title">Conceito</p></td>
			</tr>

			<tr>
				<td class="das-color-ccc">A1</td>
				<td class="das-color-ccc">A2</td>
				<td class="das-color-ccc">A3</td>
				<td class="das-color-ccc">A4</td>
				<td class="das-color-9">&nbsp;</td>
			</tr>

			<tr class="das-color-ccc">
				<td><p>6</p></td>
				<td><p>9</p></td>
				<td><p>4</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>6</p></td>
				<td><p>4</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>5</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td><p>4</p></td>
				<td><p>9</p></td>
				<td><p>4</p></td>
				<td><p>3</p></td>
				<td class="das-color-orange"><p><b>D</b></p></td>
			</tr>

			<tr class="das-color-ccc">
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>7</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>B</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>7</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>5</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td><p>7</p></td>
				<td><p>9</p></td>
				<td><p>7</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>8</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>B</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>6</p></td>
				<td><p>3</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>
			<tr class="das-color-ccc">
				<td><p>8</p></td>
				<td><p>9</p></td>
				<td><p>6</p></td>
				<td><p>5</p></td>
				<td class="das-color-orange"><p>C</p></td>
			</tr>
		</table>
		<table border="1" id="das-table-notes">
			<tr>
				<td class="das-color-orange">Nota</td>
				<td class="das-color-orange">Conceito</td>
			</tr>

			<tr class="das-color-ccc">
				<td>10-9</td>
				<td>A</td>
			</tr>
			
			<tr class="das-color-ccc">
				<td>8.9-7.5</td>
				<td>B</td>
			</tr>

			<tr class="das-color-ccc">
				<td>7.4-6</td>
				<td>C</td>
			</tr class="das-color-ccc">

			<tr class="das-color-ccc">
				<td>5.9-0.1</td>
				<td>D</td>
			</tr>

			<tr class="das-color-ccc">
				<td>0</td>
				<td>FF</td>
			</tr>


		</table>

	</div>

	<div class="das-table-activities">
		<table border="1" id="das-table-activities-names">
			<tr>
				<td colspan="2" class="das-color-orange"><p class="das-title">Integração de Mídias na Educação</p></td>
			</tr>
			<tr class="das-color-ccc">
				<td>A1</td>
				<td><p>Atividade 1 - Compreensão Inicial de uso de Mídias na Educação (Fórum 8)</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td>A2</td>
				<td><p>Atividade 2 - Diagnóstico de uso de laboratórios de Informática, Tecnologias e Mídias na Instituição Escolar</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td>A3</td>
				<td><p>Atividade 3 - Síntese Introdução à Midias na Educação</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td>A4</td>
				<td><p>Atividade 4 - Criando Cineclub sobre Cultura Digital - Compartilhando resenhas e indicações de filme sobre o tema</p></td>
			</tr>
		</table>
		
		<table border="1" id="das-table-activities-names">
			<tr class="das-color-ccc">
				<td colspan="2" class="das-color-orange"><p class="das-title">Projeto e Produção de Material</p></td>
			</tr>
			<tr class="das-color-ccc">
				<td>A1</td>
				<td><p>Atividade 1 - Compreensão Inicial de uso de Mídias na Educação (Fórum 8)</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td>A2</td>
				<td><p>Atividade 2 - Diagnóstico de uso de laboratórios de Informática, Tecnologias e Mídias na Instituição Escolar</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td>A3</td>
				<td><p>Atividade 3 - Síntese Introdução à Midias na Educação</p></td>
			</tr>

			<tr class="das-color-ccc">
				<td>A4</td>
				<td><p>Atividade 4 - Criando Cineclub sobre Cultura Digital - Compartilhando resenhas e indicações de filme sobre o tema</p></td>
			</tr>
		</table>
	</div>
		
	<div class="das-download-table">
		<p style="margin-right: 2em;">Baixar todos os dados como tabela</p>
		<select id="das-select-download">
			<option>Valores Separados por Vírgula(.csv)</option>
			<option>Microsoft Excel(.xlsx)</option>
			<option>Tabela HTML</option>
			<option>Javascript Object Notation(.json)</option>
			<option>Open Document (.ods)</option>
			</select>
		<button id="das-download">Download</button>
	</div>

</div>
</body>
</html>
